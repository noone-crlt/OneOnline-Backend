...
public class JwtService {
	...
	public String generateToken(User user) {
		return Jwts.builder().setSubject(user.getEmail()) // Tên chủ thẻ
	...
	public String extractEmail(String token) {
		return Jwts.parserBuilder().setSigningKey(getSignInKey()) // Dùng lại chìa khóa để mở
				.build().parseClaimsJws(token).getBody().getSubject(); // Lấy ra cái Subject (chính là email)
	}
}
