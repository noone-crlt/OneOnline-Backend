...
class BookAreaCodeTayApplicationTests {
		...
		try {
			...
			if (localFiles != null) {
				...
				for (File f : localFiles) {
					...
					System.out.println("  " + (isUsed ? "[KEEP]" : "[DELETE]") + " " + f.getName() + " (" + f.length()
							+ " bytes)");
					if (!isUsed) {
			...
			for (Result<Item> result : minioClient
					.listObjects(ListObjectsArgs.builder().bucket(bucket).prefix("sach/anhbia/").build())) {
				Item item = result.get();
				String objectName = item.objectName();
				boolean isUsed = usedCovers.contains(objectName);
				System.out.println(
						"  " + (isUsed ? "[KEEP]" : "[DELETE]") + " " + objectName + " (" + item.size() + " bytes)");
				if (!isUsed) {
	...
	void updateDbCoverExtensions() {
		try {
			...
			java.util.Map<String, String> mappings = java.util.Map.of("mat-biec", "sach/anhbia/mat-biec.png",
					"rung-na-uy", "sach/anhbia/rung-na-uy.png", "harry-potter-1",
					"sach/anhbia/harry-potter-va-hon-da-phu-thuy.png", "nha-gia-kim", "sach/anhbia/nha-gia-kim.png",
					"tuoi-tre-dang-gia-bao-nhieu", "sach/anhbia/tuoi-tre-dang-gia-bao-nhieu.png", "di-tim-le-song",
					"sach/anhbia/di-tim-le-song.png", "sapiens-luoc-su-loai-nguoi",
					"sach/anhbia/sapiens-luoc-su-loai-nguoi.png");
	...
	void seedDatabaseWithAllBooks() {
		try {
			...
			bookRepository.findBySlug("harry-potter-1").ifPresent(book -> {
				...
				if (!hasPdf) {
					BookEdition pdfEd = BookEdition.builder().book(book).format("EBOOK_PDF").skuCode("PDF-HP1-001")
							.originalPrice(new BigDecimal("50000")).salePrice(new BigDecimal("30000"))
							.coverObjectName("sach/anhbia/harry-potter-va-hon-da-phu-thuy.jpg")
							.fileObjectName("sach/_harry-potter-va-hon-da-phu-thuy.pdf").isActive(true).build();
			...
			List<BookData> newBooks = List.of(
					new BookData("Biểu Tượng Thất Truyền", "bieu-tuong-that-truyen", "Dan Brown",
							"sach/anhbia/bieu-tuong-that-truyen.jpg", "sach/_bieu-tuong-that-truyen.pdf"),
					new BookData("Cây Chuối Non Đi Giày Xanh", "cay-chuoi-non-di-giay-xanh", "Nguyễn Nhật Ánh",
							"sach/anhbia/cay-chuoi-non-di-giay-xanh.jpg", "sach/_cay-chuoi-non-di-giay-xanh.pdf"),
					new BookData("Chim Cổ Đỏ", "chim-co-do", "Jo Nesbo", "sach/anhbia/chim-co-do.jpg",
							"sach/_chim-co-do.pdf"),
					new BookData("Cho Tôi Xin Một Vé Đi Tuổi Thơ", "cho-toi-xin-mot-ve-di-tuoi-tho", "Nguyễn Nhật Ánh",
							"sach/anhbia/cho-toi-xin-mot-ve-di-tuoi-tho.jpg",
							"sach/_cho-toi-xin-mot-ve-di-tuoi-tho.pdf"),
					new BookData("Đi Tìm Lẽ Sống", "di-tim-le-song", "Viktor Frankl", "sach/anhbia/di-tim-le-song.png",
							"sach/_di-tim-le-song.pdf"),
					new BookData("Mật Mã Da Vinci", "mat-ma-da-vinci", "Dan Brown", "sach/anhbia/mat-ma-da-vinci.jpg",
							"sach/_mat-ma-da-vinci.pdf"),
					...
					new BookData("Ngày Xưa Có Một Chuyện Tình", "ngay-xua-co-mot-chuyen-tinh", "Nguyễn Nhật Ánh",
							"sach/anhbia/ngay-xua-co-mot-chuyen-tinh.jpg", "sach/_ngay-xua-co-mot-chuyen-tinh.pdf"),
					new BookData("Nghệ Thuật Tư Duy Chiến Lược", "nghe-thuat-tu-duy-chien-luoc", "Barry J. Nalebuff",
							"sach/anhbia/nghe-thuat-tu-duy-chien-luoc.jpg", "sach/_nghe-thuat-tu-duy-chien-luoc.pdf"),
					new BookData("Nhà Giả Kim", "nha-gia-kim", "Paulo Coelho", "sach/anhbia/nha-gia-kim.jpg",
							"sach/_nha-gia-kim.pdf"),
					new BookData("Phía Sau Nghi Can X", "phia-sau-nghi-can-x", "Higashino Keigo",
							"sach/anhbia/phia-sau-nghi-can-x.jpg", "sach/_phia-sau-nghi-can-x.pdf"),
					new BookData("Sapiens Lược Sử Loài Người", "sapiens-luoc-su-loai-nguoi", "Yuval Noah Harari",
							"sach/anhbia/sapiens-luoc-su-loai-nguoi.png", "sach/_sapiens-luoc-su-loai-nguoi.pdf"),
					new BookData("Sherlock Holmes Toàn Tập", "sherlock-holmes-toan-tap", "Arthur Conan Doyle",
							"sach/anhbia/sherlock-holmes-toan-tap.jpg", "sach/_sherlock-holmes-toan-tap.pdf"),
					new BookData("Tây Du Ký", "tay-du-ky", "Ngô Thừa Ân", "sach/anhbia/tay-du-ky.jpg",
							"sach/_tay-du-ky.pdf"),
					new BookData("Tết Ở Làng Địa Ngục", "tet-o-lang-dia-nguc", "Thảo Trang",
							"sach/anhbia/tet-o-lang-dia-nguc.jpg", "sach/_tet-o-lang-dia-nguc.pdf"),
					new BookData("Thao Túng Tâm Lý", "thao-tung-tam-ly", "Shannon Thomas",
							"sach/anhbia/thao-tung-tam-ly.jpg", "sach/_thao-tung-tam-ly.pdf"),
					new BookData("Thế Giới Ngầm Của Ripley", "the-gioi-ngam-cua-ripley", "Patricia Highsmith",
							"sach/anhbia/the-gioi-ngam-cua-ripley.jpg", "sach/_the-gioi-ngam-cua-ripley.pdf"),
					new BookData("Trường Ca Achilles", "truong-ca-achilles", "Madeline Miller",
							"sach/anhbia/truong-ca-achilles.jpg", "sach/_truong-ca-achilles.pdf"),
					new BookData("Tư Duy Ngược Dịch Chuyển Thế Giới", "tu-duy-nguoc-dich-chuyen-the-gioi", "Adam Grant",
							"sach/anhbia/tu-duy-nguoc-dich-chuyen-the-gioi.jpg",
							"sach/_tu-duy-nguoc-dich-chuyen-the-gioi.pdf"),
					new BookData("Tuổi Trẻ Đáng Giá Bao Nhiêu", "tuoi-tre-dang-gia-bao-nhieu", "Rosie Nguyễn",
							"sach/anhbia/tuoi-tre-dang-gia-bao-nhieu.jpg", "sach/_tuoi-tre-dang-gia-bao-nhieu.pdf"));
			...
			for (BookData data : newBooks) {
				...
				// 2. Create Book
				Book newBook = Book.builder().title(data.title).slug(data.slug)
						.description(data.title + " - một tác phẩm xuất sắc của " + data.authorName)
						.publisher(publisher).isActive(true).build();
				...
				BookEdition pdfEd = BookEdition.builder().book(savedBook).format("EBOOK_PDF")
						.skuCode("PDF-" + data.slug.toUpperCase() + "-001").originalPrice(new BigDecimal("50000"))
						.salePrice(new BigDecimal("30000")).coverObjectName(data.coverPath).fileObjectName(data.pdfPath)
						.isActive(true).build();
	...
	void listMinioObjects() {
		try {
			...
			Iterable<Result<Item>> results = minioClient
					.listObjects(ListObjectsArgs.builder().bucket("book-area-files").recursive(true).build());
	...
	void uploadAllLocalFilesToMinio() {
		try {
			...
			if (pdfFiles != null) {
				for (File file : pdfFiles) {
					...
					try (java.io.FileInputStream fis = new java.io.FileInputStream(file)) {
						minioClient.putObject(io.minio.PutObjectArgs.builder().bucket("book-area-files")
								.object(objectName).stream(fis, file.length(), -1).contentType(contentType).build());
			...
			// 2. Upload Covers
			File[] coverFiles = anhbiaFolder
					.listFiles((dir, name) -> name.endsWith(".jpg") || name.endsWith(".png") || name.endsWith(".webp"));
			if (coverFiles != null) {
	...
	private void downloadAndUploadToMinio(String urlStr, String objectName) throws Exception {
		...
		HttpRequest request = HttpRequest.newBuilder().uri(new URI(urlStr)).header("User-Agent",
