...
class CoverFixUtility {
	...
	private static final Set<String> CONFIRMED_GOOD = Set.of("harry-potter-va-hon-da-phu-thuy", "mat-biec",
			"nha-gia-kim", "rung-na-uy", "tuoi-tre-dang-gia-bao-nhieu");

	// User-confirmed wrong/blurry covers that MUST be re-downloaded
	private static final Set<String> CONFIRMED_BAD = Set.of("mua-do", "muu-sat", "truong-ca-achilles", "chim-co-do",
			"ngay-xua-co-mot-chuyen-tinh", "sapiens-luoc-su-loai-nguoi", "cho-toi-xin-mot-ve-di-tuoi-tho",
			"the-gioi-ngam-cua-ripley");

	// Known very small files (< 10KB) that are certainly broken
	private static final Set<String> CERTAINLY_BROKEN = Set.of("di-tim-le-song", "phia-sau-nghi-can-x");
	...
	private static final Map<String, String> GOOGLE_BOOKS_VOLUME_IDS = Map.ofEntries(
			Map.entry("bieu-tuong-that-truyen", "p6rREAAAQBAJ"), Map.entry("mat-ma-da-vinci", "IuDNEAAAQBAJ"),
			Map.entry("tet-o-lang-dia-nguc", "hUnoEAAAQBAJ"), Map.entry("thao-tung-tam-ly", "JArmEAAAQBAJ"),
			Map.entry("nghe-thuat-tu-duy-chien-luoc", "xT1cEQAAQBAJ"), Map.entry("tay-du-ky", "zWiBx7fCsaMC"),
			Map.entry("cay-chuoi-non-di-giay-xanh", "U0ZRAQAAMAAJ"), Map.entry("muu-sat", "j6KcEAAAQBAJ"),
			Map.entry("mua-do", "4sKyyCuiubYC"), Map.entry("ngay-xua-co-mot-chuyen-tinh", "dfjLEQAAQBAJ"),
			Map.entry("cho-toi-xin-mot-ve-di-tuoi-tho", "37HPEQAAQBAJ"), Map.entry("chim-co-do", "9yNYEQAAQBAJ"));
	...
	void dryRunCoverFix() {
		...
		for (BookEdition ed : allEditions) {
			...
			} else if (slug.equals("cover4") || slug.equals("cover5") || m.oldCoverObjectName != null
					&& (m.oldCoverObjectName.equals("cover4.jpg") || m.oldCoverObjectName.equals("cover5.jpg"))) {
				m.confidence = "low";
				m.status = "SKIP";
				m.source = "none";
			...
			mappings.add(m);
			System.out.printf("  [%s] %s (%s) | old=%s | new=%s | %s%n", m.status, m.slug, m.bookTitle,
					m.oldCoverObjectName, m.newCoverObjectName, m.note);
		}
	...
	void downloadCovers() {
		...
		try {
			...
			for (String slug : List.of("cay-chuoi-non-di-giay-xanh", "bieu-tuong-that-truyen", "mat-ma-da-vinci",
					"nghe-thuat-tu-duy-chien-luoc", "tay-du-ky", "tet-o-lang-dia-nguc", "thao-tung-tam-ly",
					"sherlock-holmes-toan-tap", "tu-duy-nguoc-dich-chuyen-the-gioi", "phia-sau-nghi-can-x",
					"di-tim-le-song")) {
				if (!CONFIRMED_GOOD.contains(slug)) {
	...
	void executeCoverFix() {
		...
		try {
			...
			List<BookEdition> allEditions = bookEditionRepository.findAll();
			StringBuilder backupCsv = new StringBuilder(
					"edition_id,book_title,old_cover_object_name,new_cover_object_name,updated_at\n");
			...
			for (BookEdition ed : allEditions) {
				...
				try (FileInputStream fis = new FileInputStream(coverFile)) {
					minioClient.putObject(PutObjectArgs.builder().bucket(bucketName).object(newObjectName)
							.stream(fis, coverFile.length(), -1).contentType(contentType).build());
					uploadCount++;
				...
				try {
					StatObjectResponse stat = minioClient
							.statObject(StatObjectArgs.builder().bucket(bucketName).object(newObjectName).build());
					System.out.printf("  VERIFIED %s (size=%d, type=%s)%n", newObjectName, stat.size(),
							stat.contentType());
				} catch (Exception e) {
				...
				// 5. Update DB
				backupCsv.append(String.format("%d,\"%s\",\"%s\",\"%s\",\"%s\"%n", ed.getId(), ed.getBook().getTitle(),
						oldObjectName, newObjectName, LocalDateTime.now()));
				rollbackSql.append(
						String.format("UPDATE book_editions SET cover_object_name = '%s' WHERE id = %d; -- %s%n",
								oldObjectName != null ? oldObjectName.replace("'", "''") : "", ed.getId(),
								ed.getBook().getTitle()));
			...
			if (!uploadedObjects.isEmpty()) {
				Files.writeString(OUTPUT_DIR.resolve("uploaded_objects.txt"), String.join("\n", uploadedObjects));
			}
	...
	private boolean tryOpenLibrary(HttpClient client, String slug, String isbn) {
		try {
			...
			HttpRequest req = HttpRequest.newBuilder().uri(new URI(url)).header("User-Agent", "Mozilla/5.0").GET()
					.build();
	...
	private boolean tryGoogleBooksSearch(HttpClient client, String slug) {
		...
		Map<String, String> searchQueries = Map.ofEntries(Map.entry("mua-do", "Mưa Đỏ Chu Lai"),
		...
		try {
			String url = "https://www.googleapis.com/books/v1/volumes?q="
					+ URLEncoder.encode(query, StandardCharsets.UTF_8) + "&key=" + GOOGLE_BOOKS_API_KEY
					+ "&maxResults=5";
			...
			if (resp.statusCode() == 200) {
				...
				if (thumbnail != null) {
					String hqUrl = thumbnail.replace("&edge=curl", "").replace("edge=curl&", "") + "&fife=w600";
	...
	private String extractThumbnailUrl(String json) {
		...
		if (idx == -1)
			idx = json.indexOf("\"smallThumbnail\"");
		if (idx == -1)
			return null;
	...
	private void writeMappingCsv(List<CoverMapping> mappings) throws IOException {
		StringBuilder sb = new StringBuilder(
				"pdf_file,slug,edition_id,book_title,old_cover_object_name,new_cover_object_name,local_cover_file,source,confidence,status,note\n");
		for (CoverMapping m : mappings) {
			sb.append(String.format("\"_%.pdf\",\"%s\",%d,\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\"%n",
					m.slug, m.slug, m.editionId, m.bookTitle, m.oldCoverObjectName != null ? m.oldCoverObjectName : "",
					m.newCoverObjectName != null ? m.newCoverObjectName : "",
					m.localCoverFile != null ? m.localCoverFile : "", m.source != null ? m.source : "", m.confidence,
					m.status, m.note != null ? m.note : ""));
		}
